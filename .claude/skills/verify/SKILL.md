...
13. Zwei Stufen kumulativ (Beginner + Fortgeschritten, vier befüllte Domänen je Stufe): Kompetenzpfad ist kumulativ. Beginner-Diagnose → 23 Stationen (kein Fortgeschritten sichtbar). Fortgeschritten-Diagnose (`#/pfad/kompetenz/fortgeschritten`) → 45 Stationen: Beginner-Block (1–23), dann Fortgeschritten je Domäne — Technik (24–29), Taktik (30–35), Mentales (36–40), Athletik (41–45, `belastung_steuern_regenerieren` zuletzt). Stufenübergreifende weiche Voraussetzung erscheint als „Empfohlen vorher"-Hinweis (z. B. `beinarbeit_system` → `beinarbeit`, `umschalten` → `spielziel_verstehen`, `intervallausdauer` → `durchhalten`, `vom_werkzeug_zum_system` → `warum_der_kopf_mitspielt`), sperrt nie. Cross-Sport BAD: 9 Deltas im kumulativen Pfad (Mentales/Athletik ohne Delta — mentale/athletische Anforderungen sind sportartübergreifend).
...
15. Individualpfad stufen-kumulativ (`#/pfad/individual`): Der Individualpfad filtert nach Zielfaktor *und* Diagnose-Stufe. Gleicher Faktor, zwei Diagnosen: `konzentrationskonstanz` bei Beginner → 3 (nur Beginner-Mentales), bei Fortgeschritten → 8 (kumulativ Beginner + Fortgeschritten-Mentales). `doppel_spezifische_loesungen` bei Beginner → **leer** (einziger Beleg `doppel_grundlagen` liegt auf Fortgeschritten), bei Fortgeschritten → genau `doppel_grundlagen`. Reihenfolge bleibt ziel-nah → domänen-geordnet (nicht stufen-blockiert wie der Kompetenzpfad). Themenpfad kumuliert je Domäne stufenübergreifend: Technik/Taktik = 12, Mentales = 10, Athletik = 11.
