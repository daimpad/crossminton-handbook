...
13. Zwei Stufen kumulativ (Beginner + Fortgeschritten): Kompetenzpfad ist kumulativ. Beginner-Diagnose → 23 Stationen (kein Fortgeschritten sichtbar). Fortgeschritten-Diagnose (`#/pfad/kompetenz/fortgeschritten`) → 35 Stationen: Beginner-Block (1–23), dann Fortgeschritten-Technik (24–29, `beinarbeit_system` zuletzt), dann Fortgeschritten-Taktik (30–35, `engen_satz_fuehren` zuletzt). Stufenübergreifende weiche Voraussetzung erscheint als „Empfohlen vorher"-Hinweis (z. B. `beinarbeit_system` → Beginner-`beinarbeit`, `umschalten` → Beginner-`spielziel_verstehen`), sperrt nie. Cross-Sport BAD: 9 Deltas im kumulativen Pfad; Geometrie-Delta an `beinarbeit_system` (Quadrat vs. Netz), Doppel-Delta an `doppel_grundlagen` (feste Rollen im Quadrat vs. Badminton-Rotation).
14. Fortgeschritten-Taktik über Domänengrenze: Der Taktik-Block (Station 30–35) trägt gemischte weiche Voraussetzungen — `punkt_aufbauen` verweist auf die fortgeschrittene **Technik** (`ueberkopf_clear` + `kurzes_spiel_stopp`, andere Domäne, gleiche Stufe), `smash_vorbereiten` auf `smash`. Diese Kanten *ordnen* nur: der Technik-Block steht deshalb vor dem Taktik-Block, kein Baustein wird zwischen Domäne/Stufe verschoben. Themenpfad Taktik (`#/pfad/themen/taktik`) = 12 Stationen (Beginner- dann Fortgeschritten-Taktik). Individualpfad `doppel_spezifische_loesungen` (Bereich 6, Erstaktivierung) → genau `doppel_grundlagen`. `doppel_grundlagen` im Kompetenz-Kontext mit Herkunft BAD zeigt das Delta (Basistext im Themen-Kontext); drei Taktik-Bausteine tragen `reflexionsaufgabe` (`umschalten`, `gegner_lesen_muster`, `engen_satz_fuehren`), drei einen Übungsteil.
