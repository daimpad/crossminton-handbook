...
> **Stand nach vollständigem Dreistufen-Ausbau (Beginner/Fortgeschritten/Experte), Doppel-Thema, drei Cross-Sport-Herkünften, Trainingseinheiten, Regeln-Reiter und Info-Reitern.** Neu gegenüber der vorigen Fassung: die **komplette Experten-Stufe** über alle vier Spieler-Domänen (herkunftsneutral, 0 Deltas — siehe 11.5), die Info-Reiter **„Über"** und **„Mitmachen"** samt display-only **Sprach-Anzeige** (fünf Sprachen inkl. Japanisch). Bereits zuvor abgebildet: die Dimension `spielform` (einzel/doppel), der Doppel-Themenblock, die Fortgeschritten-Blöcke, stufenübergreifende Voraussetzungen, die herkunftsreine Delta-Konvention, die Herkünfte Tennis (`TEN`) und Squash (`SQ`), der Trainingseinheiten-Bestand und der Regeln-Reiter. Details gebündelt in Abschnitt 11; vollständige Datenlage in Abschnitt 9.
...
**Im Erstausbau enthalten:** das Datenmodell (inkl. der Dimension `spielform`), die Pfad-Engine (vier Pfade plus Cross-Sport-Modifikator, plus `spielform` als Navigationsachse), der **Regeln-Reiter** und die Info-Reiter **„Über"/„Mitmachen"** (mit display-only Sprach-Anzeige), die Onboarding-Diagnostik, der Gamification-Layer in seiner Grundform, die mehrsprachige Struktur (Quellsprache Deutsch). Referenzinhalt: **alle vier Spieler-Domänen über alle drei Könnensstufen** (Beginner, Fortgeschritten, Experte) plus Trainingsgestaltung auf Trainer-Stufe, ein **Doppel-Themenblock** (Querschnitt über drei Domänen), **Cross-Sport-Deltas für drei Herkünfte** (Badminton, Tennis, Squash), ein **kuratierter Trainingseinheiten-Bestand** und die vollständigen **Spielregeln** im Regeln-Reiter.
...
- **Umgebungs-Baustein** — eigenständige Outdoor-Themen (begründete Ausnahme). Mit `bausteine.outdoor-thema.json` real und als **Typ-Filter** wirksam: Umgebungs-Bausteine sind aus Kompetenz- und Themenpfad herausgenommen und leben auf der eigenen Umgebungs-Achse (`witterung`/`untergrund`, siehe 6.7); zusätzlich erreichbar über den Individualpfad (Umgebungsanpassungs-Ziele, Bereich 5) und Deep-Link.
...
Zusätzlich zu den vier Pfaden bietet `spielform` einen eigenen Zugang: Alle `spielform:doppel`-Bausteine bilden ein zusammenhängendes **Doppel-Thema**, quer über die Domänen (Taktik, Athletik, Mentales) und — nach dem Doppel-Ausbau — über alle drei Könnensstufen. Der Default-Zugang zeigt `einzel`; das Doppel-Thema ist als eigene Ansicht wählbar. Diese Achse ist orthogonal zu Stufe und Domäne — sie filtert nach Spielform, nicht statt, sondern zusätzlich zur fachlichen Einordnung.

### 6.7 `witterung` und `untergrund` als Umgebungs-Achsen
Analog zur Spielform-Achse werden `witterung` (Wind/Sonne/Nässe/Hitze) und `untergrund` (Sand/Rasen/Asche/Kunstrasen) zu eigenen Navigationsachsen: Der Outdoor-Block (Typ `umgebungs_baustein`) bildet ein zusammenhängendes **Umgebungs-Thema**, gruppierbar nach Wetter bzw. Boden. `halle`/kein Umgebungsbezug ist der Default (kein eigenes Thema, wie `einzel`). Der Baustein-Typ wirkt als **Typ-Filter**: Umgebungs-Bausteine sind aus Kompetenz- und Themenpfad genommen und nur über diese Achse, den Individualpfad (Bereich-5-Ziele) und Deep-Link erreichbar. Kein Cross-Sport-Modifikator — Outdoor ist crossminton-eigen.
...
**Querschnitt-Themen (Doppel über drei Stufen + Outdoor):**
14. **`bausteine.doppel-beginner.json`** — 5 Bausteine, 0 Deltas, alle `spielform: doppel` (Beginner). Doppel-Grundlagen (wer nimmt den Ball, einfacher Aufschlag, Absprache, Platz lassen); gemischt 3 `uebungsteil` / 2 `reflexionsaufgabe`.
15. **`bausteine.doppel-thema.json`** — 7 Bausteine + 1 Delta, alle `spielform: doppel` (Fortgeschritten), über drei Domänen (Taktik 5, Athletik 1, Mentales 1). Führt die Dimension `spielform` ein (11.2). `doppel_grundlagen` (aus Datei 7) ist nachträglich mit `spielform: doppel` markiert.
16. **`bausteine.doppel-experte.json`** — 5 Bausteine, 0 Deltas, alle `spielform: doppel` (Experte), herkunftsneutral (11.5); gemischt 2 `uebungsteil` / 3 `reflexionsaufgabe`. Die Spielform-Achse deckt damit alle drei Stufen ab (**18** Bausteine), `doppel_spezifische_loesungen` ist stufenübergreifend belegt.
...
20. **`trainingseinheiten.json`** — 4 kuratierte Trainingseinheiten, je in drei Phasen (Erwärmung/Hauptteil/Ausklang), Referenzen per Baustein-ID auf Übungsteile (1:1). Einheiten: „Erste Schläge" und „Bewegung und Position" (Beginner), „Angriff aufbauen" (Fortgeschritten), „Als Paar spielen" (`spielform: doppel`). Trägt `kompetenzstufe` und `spielform` zur Filterung im Trainingspfad. Alle 19 Übungsteil-Referenzen gegen den Bestand validiert. Entity-Schema in der Datei unter `_meta.entity_schema`. Speist den Trainingspfad (6.4).
...
**`untergrund` als Liste** (dritte koordinierte Erweiterung, mit dem Outdoor-Block): Der bislang einwertige Untergrund wird zur **Liste** (mehrere Böden je Baustein). Ein Alt-String `"halle"` liest sich als `['halle']`, ein fehlendes Feld = Halle/Default. `untergrundVon()` normalisiert, `pruefeDaten` validiert je Listenwert. Zusammen mit `witterung` bildet `untergrund` die **Umgebungs-Achsen** (6.7); der Baustein-Typ `umgebungs_baustein` wirkt als **Typ-Filter** (aus Kompetenz-/Themenpfad genommen). Das Vokabular (Typ, Witterungs- und Untergrund-Werte, Bereich-5-Ziele) war bereits vollständig angelegt — koordiniert ergänzt wurden nur die UI-Labels der neuen Achse (`pfad_umgebung`, `umgebung_wetter`/`_boden`/`_alle`).
