...
**Experte (2 Blöcke, dritte Könnensstufe):**
...
10b. **`bausteine.experte-taktik.json`** — 6 Bausteine (2 Übungsteil, 4 Reflexionsaufgabe), 0 Deltas, ebenfalls herkunftsneutral. Höchste taktische Ebene: Matchplan, Gegner-Scouting (Archetypen + Gegenrezepte), das eigene Spiel aufzwingen, Schwächen systematisch angreifen, Matchverlauf steuern, entscheidender Punkt. Stufenübergreifende weiche Voraussetzungen auf die fortgeschrittene Taktik (`gegner_lesen_muster`, `punkt_aufbauen`, `engen_satz_fuehren`) plus interne Experten-Kanten (z. B. `entscheidender_punkt → matchverlauf_steuern`).
...
16. **`app-info.json`** — statischer App-Info-Bereich: die Reiter **„Über"** (`ueber`: Absätze, „Dank und Quellen", „Lizenz und Credits", GitHub-Link) und **„Mitmachen"** (`mitmachen`: Einleitung + drei `moeglichkeiten[]` mit `cta_label`/`cta_ziel`) plus die Konfiguration der **Sprachanzeige** (`sprachen`, `funktion_aktiv: false` — rein darstellend, Liste de/en/fr/pl/ja). Kein Lerninhalt, kein Fortschritt, nicht im Baustein-Pool. Platzhalter in `[eckigen Klammern]` (Name/Lizenz/GitHub-URL) bleiben sichtbar, bis der Betreiber sie füllt — sie werden nie erfunden oder verlinkt. Gerendert von `js/ansichten/info.js` unter `#/ueber` / `#/mitmachen`; die Sprachanzeige liegt im Kopf neben dem Hamburger (das funktionale Umschalten bleibt der Selektor im Profil).

Diese Daten machen Pfad-Engine, Modifikator, Onboarding, `spielform`-Navigation, Trainingspfad, Regeln- und Info-Reiter real durchspielbar; durch beide Ziel-Dimensionen über zwei Stufen auch der Individualpfad.

---

## 10. Offene inhaltliche Arbeit (noch nicht in den Daten)

- **Experten-Stufe außerhalb von Technik und Taktik**: die dritte Könnensstufe ist jetzt in **Technik** (`bausteine.experte-technik.json`) und **Taktik** (`bausteine.experte-taktik.json`) bespielt (beide herkunftsneutral) und macht den Kompetenzpfad dreistufig. Mentales und Athletik/Kondition auf Experten-Niveau bleiben offen.
