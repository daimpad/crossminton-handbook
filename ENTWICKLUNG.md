...
- **GitHub Pages** (eingerichtet): Jeder Push auf `main` veröffentlicht automatisch über [`.github/workflows/deploy-pages.yml`](.github/workflows/deploy-pages.yml) nach der Custom Domain `https://crossminton-handbook.de/` (eingerichtet über die `CNAME`-Datei im Repo-Root + „Custom domain" in den GitHub-Pages-Repo-Einstellungen; DNS zeigt extern auf GitHub Pages). Vorher laufen die Engine-Tests als Qualitätsschranke.
